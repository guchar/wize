...
struct ContentView: View {
    ...
    @State private var recentSearches: [String] = [] // Add this line
                ...
                VStack {
                    ...
                    if isLoading {
                        ...
                        HStack(spacing: 20) {
                            ...
                            Button(action: generateMoreUnits) {
                                Text("Dive Deeper")
                                    .fontWeight(.semibold)
                                    .foregroundColor(.white)
                                    .padding(.vertical, 12)
    ...
    func generateLesson() {
            ...
            // Check if we have saved content for this topic
            ...
            Task {
                do {
                    ...
                    DispatchQueue.main.async {
                        ...
                        self.isLoading = false
                        self.saveCurrentContent() // Save the generated content
                    }
                } catch {
                    DispatchQueue.main.async {
    ...
    // Modify the saveRecentSearches function
    ...
    func generateMoreUnits() {
        ...
        func processStreamChunk(_ chunk: String, isAdditional: Bool = false) async {
            ...
            DispatchQueue.main.async {
                if isAdditional {
                    let additionalUnits = Array(processedUnits.prefix(3))
                    self.units.append(contentsOf: additionalUnits)
                } else {
                    self.units = processedUnits
                }
                self.updateLoadingMessage()
    ...
    struct RecentSearchItem: View {
        ...
        var body: some View {
            ZStack {
                ...
                if hoveredSearch == search {
                    Button(action: { onDelete(search) }) {
                        Image(systemName: "xmark.circle.fill")
                            .foregroundColor(.white)
                            .padding(4)
                    }
